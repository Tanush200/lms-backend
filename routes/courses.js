...
// Get courses for the logged-in user (must be before any dynamic :id routes)
router.get(
  "/my-courses",
  ...
  async (req, res) => {
    try {
      ...
      if (req.user.role === "teacher") {
        ...
        // Admin/principal: all courses in their school
        if (!isSuper && userSchoolId) {
      ...
      // Constrain by school for non-super users
      if (!isSuper && userSchoolId) {
        query = { ...query, school: userSchoolId };
      }

      const courses = await Course.find(query)
        .select("title category instructor school status createdAt")
      ...
      const payload = courses.map((c) => ({
        ...c.toObject(),
        enrollmentCount: countMap.get(c._id.toString()) || 0,
      }));
